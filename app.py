...
with open(r"D:\DATA SCIENCE\Projects\Airline Customer Satisfaction\APP\Models\lgbm_model.pkl", "rb") as file:
...
def predict_satisfaction(online_boarding , delay_ratio , inflight_wifi , passenger_class , travel_type , inflight_entertainment , flight_distance , seat_comfort , leg_room_service , on_board_service , ease_online_booking , cleanliness):
    X_new = np.array([online_boarding , delay_ratio , inflight_wifi , passenger_class , travel_type , inflight_entertainment , flight_distance , seat_comfort , leg_room_service , on_board_service , ease_online_booking , cleanliness]).reshape(1,-1)
...
# Column1 - Coleecting Delay And Distance info
with col1:
...
# Column 2 - Collecting User Preferneces for Flight Service
...
with col2:
    ...
    ease_online_booking = st.selectbox('Ease of Online Booking' , list(ordinal_mapping.values()) , help='Rate the ease of booking')
    seat_comfort = st.selectbox('Seat Comfort' , list(ordinal_mapping.values()) , help='Rate the Comfort Level of seat')
    inflight_entertainment = st.selectbox('Inflight Entertainment' , list(ordinal_mapping.values()) , help='Rate the inflight entertainment Service')
    on_board_service = st.selectbox('On Board Service' , list(ordinal_mapping.values()) , help='Rathe the quality of on-board Service')
    leg_room_service = st.selectbox('Leg Room Service' , list(ordinal_mapping.values()) , help='Rate the legroom space during the inflight experience')
...
# Organising additional options in a horizontal layout
...
if st.button('Predict Satisfaction'):
    # Make the Prediction
    prediction = predict_satisfaction(online_boarding_num , delay_ratio , inflight_wifi_num , passenger_class_num , travel_type_num , inflight_entertainment_num , flight_distance , seat_comfort_num , leg_room_service_num , on_board_service_num , ease_online_booking_num , cleanliness_num)
